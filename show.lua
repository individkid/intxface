...
function showInit(list)
	...
	result = result..showIndent(1).."CopyDat, // read from src and fld\n"
	result = result..showIndent(1).."DropDat, // read from src and fld\n"
	result = result..showIndent(1).."DscdDat, // read from src\n"
	result = result..showIndent(1).."ReptDat, // read from src\n"
	result = result..showIndent(1).."InsrDat, // read from idx\n"
	result = result..showIndent(1).."PsteDat, // read from fld\n"
	result = result..showIndent(1).."ZeroDat, // read from\n"
	result = result.."};\n"
	result = result.."struct DatxField {\n"
	result = result..showIndent(1).."void *usr; // which field(s) changed\n"
	result = result..showIndent(1).."int src; // value changed\n"
	result = result..showIndent(1).."int fld; // value written\n"
...
function showMergeC(name,struct)
	local result = ""
	local depth = 1
	result = result.."void merge"..name.."(int dst, initFunc fnc, struct DatxField *arg)"
	if prototype then return result..";" end
	result = result.."\n{\n"
	...
	for ky,vl in ipairs(struct) do
		...
		if (Enumz[vl[2]]~=nil) then
			result = result..showIndent(depth).."while (1) {enum DatxEnum repl = ("..func.."); int temp = 0; int init = 0;\n"
			...
			result = result..showIndent(depth).."if (repl == DropDat) {temp = readInt(arg->src); readInt(arg->fld);}\n"
			result = result..showIndent(depth).."if (repl == DscdDat) {readInt(arg->src); temp = init;}\n"
			result = result..showIndent(depth).."if (repl == ReptDat) {readInt(arg->src); continue;}\n"
			...
			result = result..showIndent(depth).."if (repl == ZeroDat) {temp = init;}\n"
			result = result..showIndent(depth)..""..lval.." = temp; break;}\n"
		elseif (Structz[vl[2]]~=nil) then
			result = result..showIndent(depth).."while (1) {enum DatxEnum repl = ("..func.."); struct "..vl[2].." temp = {0}; struct "..vl[2].." init = {0};\n"
			result = result..showIndent(depth).."if (repl == KeepDat) {read"..vl[2].."(&"..lval..",arg->src);}\n"
			result = result..showIndent(depth).."if (repl == ReplDat) {read"..vl[2].."(&temp,arg->src); read"..vl[2].."(&"..lval..",arg->idx);}\n"
			result = result..showIndent(depth).."if (repl == CopyDat) {read"..vl[2].."(&temp,arg->src); read"..vl[2].."(&"..lval..",arg->fld);}\n"
			result = result..showIndent(depth).."if (repl == DropDat) {read"..vl[2].."(&"..lval..",arg->src); read"..vl[2].."(&temp,arg->fld);}\n"
			result = result..showIndent(depth).."if (repl == DscdDat) {read"..vl[2].."(&temp,arg->src); copy"..vl[2].."(&"..lval..",&init);}\n"
			result = result..showIndent(depth).."if (repl == ReptDat) {read"..vl[2].."(&temp,arg->src); free"..vl[2].."(&temp); continue;}\n"
			result = result..showIndent(depth).."if (repl == InsrDat) {read"..vl[2].."(&"..lval..",arg->idx);}\n"
			result = result..showIndent(depth).."if (repl == PsteDat) {read"..vl[2].."(&"..lval..",arg->fld);}\n"
			result = result..showIndent(depth).."if (repl == ZeroDat) {copy"..vl[2].."(&"..lval..",&init);}\n"
			result = result..showIndent(depth).."free"..vl[2].."(&temp); break;}\n"
		elseif (vl[2] == "Str") then
			result = result..showIndent(depth).."while (1) {enum DatxEnum repl = ("..func.."); char *temp = 0; char *init = 0;\n"
			result = result..showIndent(depth).."if (repl == KeepDat) {readStr(&"..lval..",arg->src);}\n"
			result = result..showIndent(depth).."if (repl == ReplDat) {readStr(&temp,arg->src); readStr(&"..lval..",arg->idx);}\n"
			result = result..showIndent(depth).."if (repl == CopyDat) {readStr(&temp,arg->src); readStr(&"..lval..",arg->fld);}\n"
			result = result..showIndent(depth).."if (repl == DropDat) {readStr(&"..lval..",arg->src); readStr(&temp,arg->fld);}\n"
			result = result..showIndent(depth).."if (repl == DscdDat) {readStr(&temp,arg->src); "..lval.." = init;}\n"
			result = result..showIndent(depth).."if (repl == ReptDat) {readStr(&temp,arg->src); free(temp); continue;}\n"
			result = result..showIndent(depth).."if (repl == InsrDat) {readStr(&"..lval..",arg->idx);}\n"
			result = result..showIndent(depth).."if (repl == PsteDat) {readStr(&"..lval..",arg->fld);}\n"
			result = result..showIndent(depth).."if (repl == ZeroDat) {"..lval.." = init;}\n"
			result = result..showIndent(depth).."free(temp); break;}\n"
		elseif (vl[2] == "Dat") then
			result = result..showIndent(depth).."while (1) {enum DatxEnum repl = ("..func.."); void *temp = 0; void *init = 0;\n"
			result = result..showIndent(depth).."if (repl == KeepDat) {readDat(&"..lval..",arg->src);}\n"
			result = result..showIndent(depth).."if (repl == ReplDat) {readDat(&temp,arg->src); readDat(&"..lval..",arg->idx);}\n"
			result = result..showIndent(depth).."if (repl == CopyDat) {readDat(&temp,arg->src); readDat(&"..lval..",arg->fld);}\n"
			result = result..showIndent(depth).."if (repl == DropDat) {readDat(&"..lval..",arg->src); readDat(&temp,arg->fld);}\n"
			result = result..showIndent(depth).."if (repl == DscdDat) {readDat(&temp,arg->src); "..lval.." = init;}\n"
			result = result..showIndent(depth).."if (repl == ReptDat) {readDat(&temp,arg->src); free(temp); continue;}\n"
			result = result..showIndent(depth).."if (repl == InsrDat) {readDat(&"..lval..",arg->idx);}\n"
			result = result..showIndent(depth).."if (repl == PsteDat) {readDat(&"..lval..",arg->fld);}\n"
			result = result..showIndent(depth).."if (repl == ZeroDat) {"..lval.." = init;}\n"
			result = result..showIndent(depth).."free(temp); break;}\n"
		else
			result = result..showIndent(depth).."while (1) {enum DatxEnum repl = ("..func.."); "..showTypeCF(vl[2]).." temp = {0}; "..showTypeCF(vl[2]).." init = {0};\n"
			result = result..showIndent(depth).."if (repl == KeepDat) {temp = read"..vl[2].."(arg->src);}\n"
			result = result..showIndent(depth).."if (repl == ReplDat) {read"..vl[2].."(arg->src); temp = read"..vl[2].."(arg->idx);}\n"
			result = result..showIndent(depth).."if (repl == CopyDat) {read"..vl[2].."(arg->src); temp = read"..vl[2].."(arg->fld);}\n"
			result = result..showIndent(depth).."if (repl == DropDat) {temp = read"..vl[2].."(arg->src); read"..vl[2].."(arg->fld);}\n"
			result = result..showIndent(depth).."if (repl == DscdDat) {read"..vl[2].."(arg->src); temp = init;}\n"
			result = result..showIndent(depth).."if (repl == ReptDat) {read"..vl[2].."(arg->src); continue;}\n"
			result = result..showIndent(depth).."if (repl == InsrDat) {temp = read"..vl[2].."(arg->idx);}\n"
			result = result..showIndent(depth).."if (repl == PsteDat) {temp = read"..vl[2].."(arg->fld);}\n"
			result = result..showIndent(depth).."if (repl == ZeroDat) {temp = init;}\n"
			result = result..showIndent(depth)..""..lval.." = temp; break;}\n"
		end	
		for key,val in ipairs(limits) do
