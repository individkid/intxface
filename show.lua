...
function showInit(list)
	...
	result = result..showIndent(1).."int fld; // value changed from\n"
	result = result..showIndent(1).."int idx; // value changed to\n"
	result = result.."};\n"
...
function showMergeC(name,struct)
	...
	for ky,vl in ipairs(struct) do
		...
		if (Enumz[vl[2]]~=nil) then
			...
			result = result..showIndent(depth).."if (repl == ReplDat) {readInt(src); temp = readInt(arg->idx);}\n"
			result = result..showIndent(depth).."if (repl == CopyDat) {temp = readInt(src);}\n"
			result = result..showIndent(depth).."if (repl == DscdDat) {readInt(src); temp = init;}\n"
		...
		elseif (Structz[vl[2]]~=nil) then
			...
			result = result..showIndent(depth).."if (repl == ReplDat) {read"..vl[2].."(&temp,src); read"..vl[2].."(&"..lval..",arg->idx);}\n"
			result = result..showIndent(depth).."if (repl == CopyDat) {read"..vl[2].."(&temp,src); read"..vl[2].."(&"..lval..",arg->fld);}\n"
			result = result..showIndent(depth).."if (repl == DscdDat) {read"..vl[2].."(&temp,src); copy"..vl[2].."(&"..lval..",&init);}\n"
		...
		elseif (vl[2] == "Str") then
			...
			result = result..showIndent(depth).."if (repl == ReplDat) {readStr(&temp,src); readStr(&"..lval..",arg->idx);}\n"
			result = result..showIndent(depth).."if (repl == CopyDat) {readStr(&temp,src); readStr(&"..lval..",arg->fld);}\n"
			result = result..showIndent(depth).."if (repl == DscdDat) {readStr(&temp,src); "..lval.." = init;}\n"
		...
		elseif (vl[2] == "Dat") then
			...
			result = result..showIndent(depth).."if (repl == ReplDat) {readDat(&temp,src); readDat(&"..lval..",arg->idx);}\n"
			result = result..showIndent(depth).."if (repl == CopyDat) {readDat(&temp,src); readDat(&"..lval..",arg->fld);}\n"
			result = result..showIndent(depth).."if (repl == DscdDat) {readDat(&temp,src); "..lval.." = init;}\n"
			...
			result = result..showIndent(depth).."if (repl == ReplDat) {read"..vl[2].."(src); temp = read"..vl[2].."(arg->idx);}\n"
			result = result..showIndent(depth).."if (repl == CopyDat) {read"..vl[2].."(src); temp = read"..vl[2].."(arg->fld);}\n"
			result = result..showIndent(depth).."if (repl == DscdDat) {read"..vl[2].."(src); temp = init;}\n"
