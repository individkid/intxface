...
int main(int argc, const char **argv)
{
	...
	int rdfd[count][count] = {}; int wrfd[count][count] = {}; int rdid[count] = {}; int wrid[count] = {};
	pid_t pids[count] = {}; pid_t gpid = getpid(); setpgid(0,0);
	...
	int fdes[2]; if (pipe(fdes) < 0) return -1;
	rdid[i] = fdes[0]; wrid[i] = fdes[1];}
	for (int i = 0; i < count; i++) for (int j = 0; j < count; j++) if (i == j) {
	struct Argument arg = {enums[i],enums[j],rdfd[i][j],wrid[i]};
	char *show = 0; showArgument(&arg,&show); shows[i][j] = show;} else {
	...
	pid_t pid; int status; while ((pid = wait(&status)) >= 0) for (int i = 0; i < count; i++) if (pid == pids[i]) {
	if (WEXITSTATUS(status)) {fprintf(stderr,"%s: exited abnormally\n",procs[i][0]); kill(-gpid,SIGTERM);}
