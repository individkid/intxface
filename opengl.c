...
// TODO for each gpu buffer
//  (arrayId,vertexId,elementId,uniformId,imageId,fenceId),
//  keep round robin of growing number of identifiers
//  (up to 16 sampler identifiers, up to NUMCNTX vertex ids, etc.),
//  with seqnum per identifier, indicating updatedness.
// TODO increment identifier seqnum per DMA0 command,
//  and retain pending updates until gpu buffer seqnum passes it.
// TODO figure out when to discard pendings and mark as initial
//  (perhaps too many pendings or more than half changed).
// TODO use autogenerated set get functions for containers
...
void putInt(struct bufInt *buf, int sub, int *box);
int getInt(struct bufInt *buf, INITBOX(int), int sub, int *box);
int sizInt(struct bufInt *buf);
VALUEBOX(int,Int);
void putGluint(struct bufGluint *buf, int sub, gluint *box);
